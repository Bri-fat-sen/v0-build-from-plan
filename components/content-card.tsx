  ...
  return (
    <button onClick={() => playTrack(track)}
      ...
      <div className="relative size-10 flex-shrink-0 overflow-hidden rounded">
        <SafeImage src={track.coverArt} alt={track.title} fill className="object-cover" fallbackType="music" />
        {isCurrent && isPlaying && (
  ...
  return (
    <Link href={`/music/album/${album.id}`} className="group w-40 flex-shrink-0 sm:w-44">
      <div className="relative aspect-square overflow-hidden rounded-lg">
        <SafeImage src={album.coverArt} alt={album.title} fill className="object-cover transition-transform group-hover:scale-105" fallbackType="music" />
  ...
  return (
    <Link href={`/music/artist/${artist.id}`} className="group flex w-32 flex-shrink-0 flex-col items-center sm:w-36">
      <div className="relative size-28 overflow-hidden rounded-full sm:size-32">
        <SafeImage src={artist.avatar} alt={artist.name} fill className="object-cover transition-transform group-hover:scale-105" fallbackType="avatar" />
      </div>
  ...
  return (
    <Link href={`/movies/${movie.id}`} className="group w-32 flex-shrink-0 sm:w-36">
      <div className="relative aspect-[2/3] overflow-hidden rounded-lg">
        <SafeImage src={movie.poster} alt={movie.title} fill className="object-cover transition-transform group-hover:scale-105" fallbackType="movie" />
...
export function CreatorCard({ creator }: { creator: Creator }) {
  return (
    <Link href={`/creators/${creator.id}`} className="group flex w-36 flex-shrink-0 flex-col items-center sm:w-40">
      <div className="relative size-28 overflow-hidden rounded-full sm:size-32">
        <SafeImage src={creator.avatar} alt={creator.name} fill className="object-cover transition-transform group-hover:scale-105" fallbackType="creator" />
      </div>
...
export function PlaylistCard({ playlist }: { playlist: { id: string; title: string; cover: string; curator: string; tracks: number } }) {
  return (
    <Link href={`/music/playlist/${playlist.id}`} className="group w-40 flex-shrink-0 sm:w-44">
      <div className="relative aspect-square overflow-hidden rounded-lg">
        <SafeImage src={playlist.cover} alt={playlist.title} fill className="object-cover transition-transform group-hover:scale-105" fallbackType="music" />
        <div className="absolute inset-0 flex items-end bg-gradient-to-t from-black/60 to-transparent opacity-0 transition-opacity group-hover:opacity-100">
