  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col items-center justify-center overflow-hidden bg-background">
      ...
      <div className="pointer-events-none absolute inset-0">
        ...
        <div className="absolute right-1/4 top-1/4 h-[200px] w-[200px] rounded-full bg-white/3 blur-[60px] loader-pulse" style={{ animationDelay: "1.6s" }} />
      </div>
      ...
      <div className="relative flex flex-col items-center gap-10">
        ...
        {/* Logo */}
        <div className="relative loader-logo-enter">
          {/* Blur glow behind text */}
          <div className="absolute inset-0 -z-10 flex scale-110 blur-2xl opacity-40 select-none pointer-events-none">
        ...
        <div className="flex items-end gap-[5px]">
          {[0, 1, 2, 3, 4, 5, 6].map(i => (
            <div
              ...
              style={{
                background: i < 4
                  ? `hsl(var(--primary) / ${0.5 + i * 0.15})`
                  : `hsl(var(--primary) / ${1 - (i - 3) * 0.2})`,
        ...
        {/* Message */}
        <p className="loader-fade-in font-mono text-xs tracking-widest text-muted-foreground uppercase" style={{ animationDelay: "0.4s" }}>
          {message ? `${message}${dots}` : `Loading${dots}`}
        </p>

        {/* Progress bar */}
        {showProgress && (
          <div className="loader-fade-in w-48 md:w-64 space-y-2" style={{ animationDelay: "0.5s" }}>
            ...
            <p className="text-center font-mono text-xs text-muted-foreground">{progress}%</p>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="flex min-h-[300px] flex-col items-center justify-center gap-6 py-16">
      ...
      <div className="flex items-end gap-1">
        {[0, 1, 2, 3, 4].map(i => (
          <div key={i} className="eq-bar w-1 rounded-full bg-primary" style={{ animationDelay: `${i * 0.12}s` }} />
        ))}
      </div>
      {message && <p className="font-mono text-xs tracking-widest text-muted-foreground uppercase">{message}</p>}
    </div>
  )
}
...
export function SkeletonHero({ className }: { className?: string }) {
  return (
    <div className={cn("animate-pulse rounded-2xl bg-white/[0.04] relative overflow-hidden", className)}>
    ...
    <div className={cn("space-y-3", className)}>
      ...
      {cards > 0 && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
          {[...Array(cards)].map((_, i) => (
            <SkeletonCard key={i} className={`animation-delay-${i}`} />
          ))}
        </div>
      )}
      {rows > 0 && (
